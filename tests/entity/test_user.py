...
def test_user_creation_strips_email_whitespace() -> None:
    """Arrange-Act-Assert: 이메일 앞뒤 공백은 제거된다."""
    # Arrange
    raw_email = "  player@example.com  "

    # Act
    user = User(user_id="u1", display_name="P", email=raw_email)
...
def test_user_is_immutable() -> None:
    """Arrange-Act-Assert: User 엔티티는 생성 후 변경할 수 없다."""
    # Arrange
    user = User(user_id="u1", display_name="Player")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("u" * 65, "Name", None, "user_id"),
        ("id", "n" * 121, None, "display_name"),
        ("id", "Name", f"{'a' * 250}@x.com", "email"),
    ],
)
